...
public class AgressiveMachine extends Machine {
     ...
     * @param name
     */
    public AgressiveMachine(String name, Gomoku gomoku){
    	super(name, gomoku);
    ...
    public void play(int xPosLast, int yPosLast, int dimensiones) throws GomokuException {
    	if(!(xPosLast + 1 >= dimensiones) && Gomoku.getGomoku().validPlay(xPosLast + 1, yPosLast)) {
    		Gomoku.getGomoku().play(xPosLast + 1, yPosLast);
    	}else if(!(xPosLast - 1 < 0) && Gomoku.getGomoku().validPlay(xPosLast - 1, yPosLast)) {
    		Gomoku.getGomoku().play(xPosLast - 1, yPosLast);
    	}else if(!(yPosLast - 1 < 0) && Gomoku.getGomoku().validPlay(xPosLast, yPosLast - 1)){
    		Gomoku.getGomoku().play(xPosLast, yPosLast-1);
    	}else if(!(yPosLast + 1 >= dimensiones) && Gomoku.getGomoku().validPlay(xPosLast, yPosLast + 1)) {
    		Gomoku.getGomoku().play(xPosLast, yPosLast+1);
    	}
    }
